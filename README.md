...
- Ensure both the account for `CLIENT` and `CONTRIBUTOR` have enough MATIC to pay for gas
- Should be no more than a 2-3 MATIC
...
__*Note*__ the `OPENQ_ADDRESS` output to `.env.contracts`

### Update OpenQV0 Addresses
...
### Verify the OpenQ Implementation Contract
...
### Verify the OpenQ UUPS Proxy Contract

NOTE: For the OpenZeppelin Proxy you must downgrade `hardhat.config.js` to `solidity: '0.8.2'`

| Environment      | Deploy Script |
| ----------- | ----------- |
| Development      | ``npx hardhat verify OPENQ_PROXY_ADDRESS --network mumbai --contract ``       |
| Staging      | ``npx hardhat verify OPENQ_PROXY_ADDRESS --network polygon --contract ``       |
| Production   | ``npx hardhat verify --contract node_modules/@openzeppelin/contracts/proxy/ERC1967/ERC1967Proxy.sol --network polygon 0x994EDF928B9306F88339C08a1B37424719cc09C6 0x6E95704e9102289361276CF5DA5f795462D43235 0x``        |
